<?php
// fitgymtrack_flutter/Api server/firebase/send_push_notification_legacy.php
...
try {
    ...
    if ($is_broadcast) {
        // Broadcast a tutti gli utenti della palestra
    ...
    // Invia notifica push usando Legacy API
    $push_results = [];
    foreach ($fcm_tokens as $token) {
        $push_result = sendFCMNotificationLegacy($token, $title, $message, $type, $priority);
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Notifica push inviata con successo (Legacy API)',
...
/**
 * Invia notifica FCM usando Legacy API
 */
function sendFCMNotificationLegacy($token, $title, $message, $type, $priority) {
    // Server Key dal file google-services.json
    $server_key = 'AIzaSyDsC_d7wgfyFXAMR5uVXXw1XHYikNX98Gc';
