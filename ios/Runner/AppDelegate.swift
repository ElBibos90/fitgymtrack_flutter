...
@objc class AppDelegate: FlutterAppDelegate {
  ...
  private func setupPushNotifications(_ application: UIApplication) {
    ...
    UNUserNotificationCenter.current().requestAuthorization(
      ...
      completionHandler: { granted, error in
        ...
        if let error = error {
          "🔔 [iOS] Notification permission error: \(error)")
        }
      }
    )
  ...
  override func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
    "🔔 [iOS] Failed to register for remote notifications: \(error)")
  }
}
...
extension AppDelegate {
  ...
  override func userNotificationCenter(_ center: UNUserNotificationCenter,
    ...
    "🔔 [iOS] Foreground notification received: \(userInfo)")
  ...
  override func userNotificationCenter(_ center: UNUserNotificationCenter,
    ...
    "🔔 [iOS] Notification tapped: \(userInfo)")
  ...
  private func handleNotificationTap(_ userInfo: [AnyHashable: Any]) {
    ...
    if let notificationId = userInfo["notification_id"] as? String {
      "🔔 [iOS] Opening notification: \(notificationId)")
...
extension AppDelegate: MessagingDelegate {
  ...
  func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
    "🔔 [iOS] FCM registration token: \(fcmToken ?? "nil")")
  ...
  private func sendTokenToServer(_ token: String) {
    ...
    "🔔 [iOS] Sending FCM token to server: \(token)")
