...
class _SimpleActiveWorkoutScreenState extends State<SimpleActiveWorkoutScreen> {
  ...
  void initState() {
    super.initState();
    debugPrint("🔥 [V4 HTTP] initState called");
  ...
  @override
  void dispose() {
    debugPrint("🔥 [V4 HTTP] dispose called");
    _saveWorkoutState();
    ...
    try {
      ...
      _dio.interceptors.add(InterceptorsWrapper(
        onRequest: (options, handler) {
          debugPrint("🔥 [V4 HTTP] Request: ${options.method} ${options.uri}");
          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint("🔥 [V4 HTTP] Response: ${response.statusCode}");
          handler.next(response);
        },
        onError: (error, handler) {
          debugPrint("🔥 [V4 HTTP] Error: ${error.message}");
          handler.next(error);
        },
      ));
      ...
      debugPrint("🔥 [V4 HTTP] HTTP client initialized successfully");
    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error initializing HTTP: $e");
      setState(() {
  ...
  Future<void> _initializeWorkout() async {
    ...
    try {
      ...
      debugPrint("🔥 [V4 HTTP] Error initializing: $e");
    } finally {
      setState(() {
  ...
  Future<void> _loadExercisesFromApi() async {
    try {
      debugPrint("🔥 [V4 HTTP] Loading exercises from API...");
      setState(() {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("🔥 [V4 HTTP] Successfully loaded ${_exercises.length} exercises");
      } else {
        throw Exception('HTTP ${response.statusCode}');
      }
    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error loading exercises: $e");
  ...
  Future<void> _saveSeriesViaApi(int seriesNumber) async {
    try {
      debugPrint("🔥 [V4 HTTP] Saving series to API...");
      ...
      if (response.statusCode == 201) {
        ...
        debugPrint("🔥 [V4 HTTP] Series saved successfully via API");
      } else {
        throw Exception('Save failed: HTTP ${response.statusCode}');
      }

    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error saving series via API: $e");
      setState(() {
  ...
  Future<void> _restoreWorkoutState() async {
    try {
      ...
      debugPrint("🔥 [V4 HTTP] Restoring state: ${stateJson ?? 'null'}");
      ...
      if (stateJson != null) {
        ...
        debugPrint("🔥 [V4 HTTP] State restored: $_completedSeries series, exercise $_currentExerciseIndex");
      } else {
        _startTime = DateTime.now();
      }
    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error restoring state: $e");
      _startTime = DateTime.now();
    }
  }
  ...
  Future<void> _saveWorkoutState() async {
    try {
      ...
      debugPrint("🔥 [V4 HTTP] State saved with $_apiCalls API calls");
    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error saving state: $e");
    }
  }
  ...
  Future<void> _clearWorkoutState() async {
    try {
      ...
      debugPrint("🔥 [V4 HTTP] State cleared");
    } catch (e) {
      debugPrint("🔥 [V4 HTTP] Error clearing state: $e");
    }
  }
  ...
  Future<void> _handleCompleteSeries() async {
    debugPrint("🔥 [V4 HTTP] handleCompleteSeries called");
